 ...
 ****************************************************************************/
extern uint8_t pc_out_report[64];
extern uint8_t pc_in_report[64];
...
static ErrorCode_t HID_Ep_Hdlr(USBD_HANDLE_T hUsb, void *data, uint32_t event)
{
	...
	case USB_EVT_OUT:
		...
		USBD_API->hw->ReadEP(hUsb, pHidCtrl->epout_adr, pc_out_report);
		break;
	}
	return LPC_OK;
}
